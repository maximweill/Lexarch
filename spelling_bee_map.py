    ...
    for _, row in df.iterrows():
        ...
        if len(syls) == len(prons):
            for s, p in zip(syls, prons):
                sig = f"{s} ({p})"
                rows.append([word, sig, diff, freq])
...
# -----------------------------------------------------------------------------
# 3. UI CONFIGURATION ("OLD BOOK" THEME)
# -----------------------------------------------------------------------------
custom_css = """
<style>
...
def server(input, output, session):
    ...
    def similar_treemap():
        ...
        fig = px.treemap(
            matched_df, path=['Signature', 'Word'], values='Frequency',
            color='Difficulty', color_continuous_scale='RdYlGn_r', range_color=[0, 1]
        )
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#1a1a1a', family="Lora, serif"), margin=dict(t=0, l=0, r=0, b=0))
        return fig
    ...
    def ui_step4_selection():
        ...
        for i in range(n):
            ...
            if row.empty:
                ui_elements.append(ui.p(f"❌ '{w_clean}' not found.", style="color:#d32f2f"))
                continue
            ...
            ui_elements.append(ui.h5(f"{w_clean}", style="color:#1a1a1a; margin-bottom:5px; border-bottom:1px solid #ddd;"))
        ...
        for i in range(n):
            try:
                ...
                for idx_str in selected_indices:
                    idx = int(idx_str)
                    s = syls[idx]
                    word_map[s] = prons[idx]
                    user_selected_syllables.add(s)
    ...
    def submit_round():
        ...
        for i, word in enumerate(words):
            ...
            val_clean = str(val).strip().upper() # Safe convert
